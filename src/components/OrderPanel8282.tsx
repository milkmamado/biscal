...
const OrderPanel8282 = ({ symbol, onPositionChange, onPnLChange, onOpenOrdersChange, onTradeClose, onTpSlChange, onOrderBookChange, dailyLossKRW = 0, dailyProfitPercent = 0 }: OrderPanel8282Props) => {
  ...
  // 누적 거래시간 관리 (당일 합산, 다음날 오전 9시 리셋)
  const TRADING_TIME_KEY = 'dailyTradingTime';
  const TRADING_TIME_LIMIT_MS = 60 * 60 * 1000; // 1시간 (밀리초)
  
  // 현재 거래일 ID 반환 (오전 9시 기준으로 날짜 변경)
  const getTradingDayId = (): string => {
    const now = new Date();
    ...
    // 오전 9시 이전이면 전날로 취급 (밤 11시 이후 접속 → 전날 시간에 합산)
    if (hour < 9) {
      const prevDay = new Date(koreaTime);
  ...
  const getDailyTradingTime = (): number => {
    const stored = localStorage.getItem(TRADING_TIME_KEY);
    if (!stored) return 0;
    try {
      const data = JSON.parse(stored);
      if (data.tradingDay === getTradingDayId()) {
        return data.totalMs || 0;
      }
    } catch {}
    return 0; // 새 거래일이면 0부터 시작
  };
  
  const [cumulativeTradingTimeMs, setCumulativeTradingTimeMs] = useState<number>(getDailyTradingTime());
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (tradingStartTimeRef.current) {
        ...
        const newTotal = getDailyTradingTime() + elapsed;
