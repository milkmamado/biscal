...
export function useAutoTrading({ balanceUSD, leverage, krwRate, onTradeComplete, initialStats, logTrade }: UseAutoTradingProps) {
  ...
  const toggleAutoTrading = useCallback(() => {
    setState(prev => {
      ...
      if (newEnabled) {
        ...
        initAudio();
        toast.success('🤖 자동매매 시작 (BB+급등 모드)');
      } else {
        toast.info('자동매매 중지');
      }
      return { 
        ...
        pendingSignal: null,
        statusMessage: newEnabled ? '🔍 BB+급등 동시 시그널 검색 중...' : '자동매매 비활성화',
      };
    });
  }, []);
  ...
  const executeEntry = useCallback(async (
    ...
    try {
      ...
      lastEntryTimeRef.current = Date.now();
      setState(prev => ({ ...prev, pendingSignal: null, statusMessage: '🔍 BB+급등 동시 시그널 검색 중...' }));
      addLog({
        symbol,
        action: 'error',
        side,
  ...
  const closePosition = useCallback(async (reason: 'tp' | 'sl' | 'exit', currentPrice: number) => {
    ...
    try {
      ...
      if (!actualPosition) {
        setState(prev => ({ ...prev, currentPosition: null, currentSymbol: null, statusMessage: '🔍 BB+급등 동시 시그널 검색 중...' }));
        addLog({
